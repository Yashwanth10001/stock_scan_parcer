...
class StockBloc extends Bloc<StockEvent, StockState> {
  ...
  FutureOr<void> stockInitialEvent(
      StockInitialEvent event, Emitter<StockState> emit) {}
  ...
  getFormattedString(Criteria criteria) {
    ...
    for (int i = 1; i < values.length; i++) {
      ...
      if (criteria.variable[val] != null) {
        if (criteria.variable[val]['type'] != null &&
            criteria.variable[val]['type'] == "value") {
          String modifiedString = finalString.replaceAll(
              val, '~(${criteria.variable[val]['values'][0]})~');
          finalString = modifiedString;
        } else if (criteria.variable[val]['type'] != null &&
            criteria.variable[val]['type'] == "indicator") {
          String modifiedString = finalString.replaceAll(
              val, '~(${criteria.variable[val]['default_value']})~');
          finalString = modifiedString;
        }
      }
    }
  ...
  List<num> getValuesData(Criteria criteria, String tag) {
    List<num> dataValues = [];
    dataValues..clear();
    List<String> values = criteria.text.split('\$');
    dataValues.clear();
    ...
    for (int i = 1; i < values.length; i++) {
      ...
      if (criteria.variable[val] != null) {
        if (criteria.variable[val]['type'] != null &&
          ...
          dataValues.sort((a, b) => a.compareTo(b));
          if (tag.toLowerCase().contains('bullish')) {
  ...
  bool checkData(Criteria criteria, String val) {
    ...
    for (int i = 1; i < values.length; i++) {
      ...
      if (criteria.variable[val] != null) {
        if (criteria.variable[val]['type'] != null &&
            criteria.variable[val]['type'] == "indicator") {
          if (criteria.variable[val]['default_value']
              .toString()
              .contains(modifiedVal)) {
            return true;
          }
        }
      }
